...
class Grid {
  ...
  collectNeighborTrees(treeLocation){
    ...
    if(this.grid[treeLocation[0]][treeLocation[1]] !== ""){
      ...
      for(let n = 0; n < NEIGHBORS.length; n++){
        ...
        if(this.isInBounds(neighborLocation) && this.grid[neighborLocation[0]][neighborLocation[1]] !== '\ud83d\udd25'){ // also check if tree is burning/burnt
  ...
  //  while(!this.allBurned()){
      // remove current tree from list of trees, it should be first one, no?
      const burntTree = this.burningTrees.shift();
      if(burntTree){
